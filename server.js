...
const localEnvFiles=['.env','1.env'].map(name=>path.join(root,name)).filter(file=>fs.existsSync(file));
for(const localEnvFile of localEnvFiles)for(const line of fs.readFileSync(localEnvFile,'utf8').split(/\r?\n/)){const m=line.match(/^([^#=]+)=(.*)$/);if(m&&!process.env[m[1]])process.env[m[1]]=m[2].replace(/^['\"]|['\"]$/g,'');}
...
server.listen(port,host,()=>console.log(`gopathway.tech demo listening on ${host}:${port}`));
