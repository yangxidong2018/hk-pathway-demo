...
async function requestLinkAi({message,userId,sessionId},env){
  const response=await fetch(env.LINKAI_API_URL||LINKAI_URL,{
    ...
    body:JSON.stringify({
      ...
      // The workflow owns its fixed instructions and reads business data with
      // user_id.  This field must be the user's wording exactly as entered.
      args:{input_text:String(message||''),user_id:userId},
...
async function linkAiWorkflow(payload,env){
  try{
    ...
    try{
      console.warn('linkai-request-retry',{userId:payload.userId,status:error.status});
      // A retry is still part of the same user conversation. Do not create a
      // hidden second LinkAI session that could lose conversation continuity.
      return await requestLinkAi(payload,env);
    }catch(retryError){
...
export async function chatWithAgent({message,context,history=[],userId,sessionId,locale='zh-CN'},env){
  if(env.AI_PROVIDER==='linkai'&&env.LINKAI_API_KEY&&env.LINKAI_APP_CODE){
    // LinkAI is the sole reader of assessment and recommendation data. Its
    // workflow resolves those records by user_id, so input_text must remain
    // the user's original wording rather than a duplicated data snapshot.
