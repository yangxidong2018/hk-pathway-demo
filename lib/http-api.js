...
import {completedHome,submitCrossBorder,stateOptions,providerPool,hydratePhotos} from './cross-border-store.js';
import {eligibleProfessionals} from './cross-border-matching.js';
...
export async function handleApi(req,res,env=process.env){
  ...
  try{
    ...
    if(crossEnabled&&['/api/home','/api/company','/api/assess/cross-border','/api/providers'].some(p=>url.pathname===p)||crossEnabled&&url.pathname.startsWith('/api/providers/')){
      ...
      if(req.method==='GET'&&url.pathname==='/api/home')return json(res,200,await completedHome(env,userId));
      ...
      if(req.method==='GET'&&url.pathname.startsWith('/api/providers')){
       ...
       const h=await completedHome(env,userId),a=h.result?.answers||{};
       return p?json(res,200,{provider:{...p,professionals:await hydratePhotos(env,eligibleProfessionals(p,pool.people,a).slice(0,3))}}):json(res,404,{error:'未找到服务商'});
      }
    }
    ...
    if(req.method==='POST'&&url.pathname==='/api/chat'){
      ...
      try{
       ...
       const current=crossEnabled?await completedHome(env,effectiveUserId):null;
       const context=crossEnabled?{result:current?.result||null,recommendations:current?.recommendations||[],scope:'仅跨国注册公司；界面语言不决定目标国家；只能解释既有结论，不能修改分数或推荐'}:b.context;
