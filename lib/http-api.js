...
import {crossEnv} from './cross-border-fields.js';
import {completedHome,submitCrossBorder,stateOptions,providerPool,assessmentHistory,assessmentDetail} from './cross-border-store.js';
import {softConfirmations} from '../public/cross-border.js';
import {tencentAsrReady,transcribeTencentAudio} from './tencent-asr.js';
...
const body=async(req,limit=1e6)=>{
  if(req.body&&typeof req.body==='object')return req.body;
  if(typeof req.body==='string')return req.body?JSON.parse(req.body):{};
  let s='';
  for await(const c of req)s+=c;
  if(s.length>limit)throw new Error('请求过大');
  return s?JSON.parse(s):{};
};

const secret=env=>env.AUTH_SECRET||env.FEISHU_APP_SECRET||'gopathway-local-demo';
...
// Repeated logins for one phone use one candidate ID, including across instances.
const createUserId=(phone,env)=>`USR${sign(`customer:${phone}`,secret(env)).replace(/[^a-zA-Z0-9]/g,'').slice(0,20).toUpperCase()}`;
const qaLatestAssessments=new Map();
const identitySyncs=new Map();
const voiceAttempts=new Map();
const allowVoiceAttempt=userId=>{
 const now=Date.now(),key=String(userId||''),recent=(voiceAttempts.get(key)||[]).filter(time=>now-time<60_000);
 if(recent.length>=8)return false;
 recent.push(now);voiceAttempts.set(key,recent);return true;
};
...
async function repairCustomerIdentity(env,session){
 ...
 try{
  ...
  if(!customer){
   const created=await upsertCustomer(env,session.phone,{'用户ID':session.userId});
   customer=created?.record||created;
  }
  const linked=plainTextField(customer?.fields?.['用户ID']);
  if(!linked){
   const updated=await upsertCustomer(env,session.phone,{'用户ID':session.userId});
   customer=updated?.record||updated||customer;
  }
...
async function homePayload(env,session,preferredType=''){
 if(session.qa){
  ...
  if(!saved||saved.type!=='pre-hk'||preferredType&&preferredType!=='pre-hk')return {mode:'new',assessmentType:'pre-hk',reason:'qa-assessment-missing'};
...
export async function handleApi(req,res,env=process.env){
  const crossEnabled=env.CROSS_BORDER_ENABLED==='true';
  if(crossEnabled)env=crossEnv(env);
  try{
    const url=new URL(req.url,'http://localhost');
    if(req.method==='GET'&&url.pathname==='/api/cross-border/config')return json(res,200,{enabled:crossEnabled,states:crossEnabled?await stateOptions(env):[]});
    ...
    if(req.method==='POST'&&url.pathname==='/api/login'){
      ...
      const userId=createUserId(b.phone,env);
    ...
    if(req.method==='POST'&&url.pathname==='/api/voice/transcribe'){
      if(!allowVoiceAttempt(session.userId))return json(res,429,{error:'语音请求过于频繁，请稍后再试'});
      const b=await body(req,4_200_000),locale=['zh-CN','zh-HK','en','vi','pt-BR'].includes(b.locale)?b.locale:'zh-CN';
      if(!tencentAsrReady(env))return json(res,503,{error:'语音转写服务尚未配置'});
      try{return json(res,200,{text:await transcribeTencentAudio(env,{audioBase64:b.audio,locale,format:b.format})});}
      catch(error){console.error('voice-transcription-failed',{code:error.code||'unknown'});return json(res,502,{error:error.message||'语音转写失败'});}
    }
    if(crossEnabled&&['/api/home','/api/company','/api/assess/cross-border','/api/providers','/api/assessments/history'].some(p=>url.pathname===p)||crossEnabled&&url.pathname.startsWith('/api/providers/')||crossEnabled&&url.pathname.startsWith('/api/assessments/')){
      const identity=await ensureCustomerIdentity(env,session);
      if(!identity.ok)return json(res,503,{error:'账号关联尚未完成，请稍后重试'});
      const userId=identity.effectiveUserId||session.userId;
      if(req.method==='GET'&&url.pathname==='/api/home')return json(res,200,await completedHome(env,userId,url.searchParams.get('country')||''));
      if(req.method==='GET'&&url.pathname==='/api/assessments/history')return json(res,200,{assessments:await assessmentHistory(env,userId)});
      if(req.method==='GET'&&url.pathname.startsWith('/api/assessments/'))return json(res,200,await assessmentDetail(env,userId,decodeURIComponent(url.pathname.split('/').pop())));
      if(url.pathname==='/api/company')return json(res,405,{error:'企业信息仅在完成评估后同步，请通过评估修改'});
      if(req.method==='POST'&&url.pathname==='/api/assess/cross-border'){
       const b=await body(req);if(softConfirmations(b.answers||{}).length&&!b.confirmedWarnings)return json(res,400,{error:'请先确认本次答案中的提示事项'});try{return json(res,200,await submitCrossBorder(env,{userId,phone:session.phone,locale:b.locale},b.answers||{},b.requestId));}catch(error){return json(res,error.validation?400:502,{error:error.message,validation:error.validation||[]});}
      }
      if(req.method==='GET'&&url.pathname.startsWith('/api/providers')){
       const pool=await providerPool(env);if(url.pathname==='/api/providers')return json(res,200,{providers:pool.providers});
       const p=pool.providers.find(x=>x.recordId===decodeURIComponent(url.pathname.split('/').pop()));
       // 顾问属于一次具体的推荐方案。详情页只读取该方案已写入的关联，
       // 绝不能根据用户全局最新评估再次筛选，否则切换国家时会串到别国顾问。
       const h=await completedHome(env,userId,url.searchParams.get('country')||'');
       const match=(h.recommendations||[]).find(x=>(x.providerRecordId||x.provider?.recordId)===p?.recordId);
       return p?json(res,200,{provider:{...p,professionals:match?.professionals||[]}}):json(res,404,{error:'未找到服务商'});
      }
    }
    if(crossEnabled&&req.method==='POST'&&url.pathname.startsWith('/api/assess/'))return json(res,410,{error:'旧评估接口已停用，请刷新页面后重新进入跨境公司注册评估'});
    ...
    if(req.method==='POST'&&url.pathname.startsWith('/api/assess/')){
      ...
      if(type==='in-hk')return json(res,404,{error:'在港评估暂未开放'});
    ...
    if(req.method==='POST'&&url.pathname==='/api/chat'){
      ...
      if(message.length>4000)return json(res,400,{error:'问题内容过长'});
       let sessionId=`gopathway:${session.userId}`;
      try{
       ...
       sessionId=`gopathway:${effectiveUserId}`;
       // LinkAI resolves its own business context from Feishu with user_id.
       // Keep server-side context only for non-LinkAI fallback providers.
       const requiresServerContext=env.AI_PROVIDER!=='linkai';
       const current=requiresServerContext&&crossEnabled?await completedHome(env,effectiveUserId):null;
       const context=requiresServerContext&&crossEnabled?{result:current?.result||null,recommendations:current?.recommendations||[],scope:'仅跨国注册公司；界面语言不决定目标国家；只能解释既有结论，不能修改分数或推荐'}:undefined;
       const rawReply=await chatWithAgent({...b,context,message,userId:effectiveUserId,sessionId},env);
       const parsed=parseAgentAction(rawReply,identity.customer?.fields?.['用户状态']||'');
       if(parsed.action?.type==='assess_in_hk')parsed.action=null;
       if(parsed.action?.type==='assessment_choice')parsed.action={type:'assess_pre_hk'};
    ...
    if(req.method==='GET'&&url.pathname==='/api/chat/history'){
      try{const identity=session.qa?{ok:true,customer:null,effectiveUserId:session.userId}:await ensureCustomerIdentity(env,session);if(!identity.ok)throw new Error('账号同步失败');const effectiveUserId=identity.effectiveUserId||session.userId,sessionId=`gopathway:${effectiveUserId}`;const raw=session.qa?[]:await chatHistory(env,effectiveUserId,sessionId);const messages=raw.map(m=>{if(m.role!=='assistant')return m;const parsed=parseAgentAction(m.content,identity.customer?.fields?.['用户状态']||'');if(parsed.action?.type==='assess_in_hk')parsed.action=null;if(parsed.action?.type==='assessment_choice')parsed.action={type:'assess_pre_hk'};return {...m,content:parsed.reply,action:parsed.action}});return json(res,200,{sessionId,messages});}catch(e){return json(res,502,{error:e.message||'对话记录读取失败'});}
    }
