...
import {guidesForAssessment} from './cross-border-guides.js';
...
export async function completedHome(e,userId,targetCountry=''){
 ...
 // time is written last and is the cross-device completion boundary. For an
 // older destination, a linked recommendation is the durable completion proof.
 const all=await listRecords(e,TABLES.assessment);
 const candidates=all.filter(x=>x.fields['用户ID']===userId&&(!country||x.fields['目标国家/地区']===country)&&Number(x.fields['评估提交时间'])<=completedAt).sort((a,b)=>Number(b.fields['评估提交时间'])-Number(a.fields['评估提交时间']));
 if(!candidates.length)return {mode:'new'};
 const recommendationRows=await listRecords(e,TABLES.recommendation);
 const latestCountry=customer?.fields['最近评估目标国家/地区'];
 const record=country&&country!==latestCountry?candidates.find(x=>recommendationRows.some(row=>relationIds(row.fields['关联跨境评估']).includes(x.record_id))):candidates[0];
