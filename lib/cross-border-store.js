...
import {guidesForAssessment,guidesForCountry} from './cross-border-guides.js';
...
const home=(result,recommendations=[],guides=[])=>({mode:'personalized',assessmentType:'cross-border',type:'cross-border',targetCountry:result.targetCountry,title:result.title,summary:result.summary,result,recommendations,guides,updatedAt:result.completedAt,actions:result.nextSteps.map((title,i)=>({id:`step-${i}`,title,reason:i===0?result.lightStart:'',providers:i===0?recommendations.map(x=>({...x,provider:{...x.provider,professionals:x.professionals}})):[]}))});
const hydratedHome=async(e,result,recommendations)=>{
 const [hydratedRecommendations,guides]=await Promise.all([
  Promise.all(recommendations.map(async r=>({...r,professionals:await hydratePhotos(e,r.professionals||[])}))),
  guidesForAssessment(e,result)
 ]);
 return home(result,hydratedRecommendations,guides);
};
const countryGuideHome=async(e,targetCountry)=>({
 mode:'new',
 assessmentType:'cross-border',
 type:'cross-border',
 targetCountry,
 guides:await guidesForCountry(e,targetCountry)
});

const completedRecords=async(e,userId,targetCountry='')=>{
 const customer=await findByField(e,TABLES.customer,'用户ID',userId),completedAt=Number(customer?.fields['最近评估时间']);
 if(!completedAt)return {customer,completedAt,records:[]};
 const country=String(targetCountry||'').trim(),all=await listRecords(e,TABLES.assessment);
 const records=all.filter(x=>x.fields['用户ID']===userId&&(!country||x.fields['目标国家/地区']===country)&&Number(x.fields['评估提交时间'])<=completedAt).sort((a,b)=>Number(b.fields['评估提交时间'])-Number(a.fields['评估提交时间']));
 return {customer,completedAt,records};
};
const historyItem=record=>({
 id:String(record.fields['跨境评估ID']||''),
 targetCountry:String(record.fields['目标国家/地区']||''),
 conclusion:String(record.fields['评估结论']||''),
 title:String(record.fields['结果主标题']||''),
 summary:String(record.fields['个性化结论']||''),
 completedAt:Number(record.fields['评估提交时间'])||0
});
const hydrateAssessmentRecord=async(e,record,completedAt)=>{
 const recommendationRows=await listRecords(e,TABLES.recommendation);
 ...
 const result={type:'cross-border',targetCountry:answers.F00,answers,id:record.fields['跨境评估ID'],completedAt:Number(record.fields['评估提交时间'])||completedAt,score:record.fields['评估总分'],conclusion:record.fields['评估结论'],dimensions:{}};
 ...
 const {providers,people}=await providerPool(e),rows=recommendationRows.filter(x=>relationIds(x.fields['关联跨境评估']).includes(record.record_id));
...
export async function assessmentHistory(e,userId){
 assertNewBase(e);const {records}=await completedRecords(e,userId);
 return records.map(historyItem).filter(item=>item.id&&item.targetCountry&&item.completedAt);
}

export async function assessmentDetail(e,userId,assessmentId){
 assertNewBase(e);const {completedAt,records}=await completedRecords(e,userId);
 const record=records.find(item=>String(item.fields['跨境评估ID']||'')===String(assessmentId||''));
 if(!record)throw Error('未找到这条已完成评估记录');
 return hydrateAssessmentRecord(e,record,completedAt);
}

export async function completedHome(e,userId,targetCountry=''){
 assertNewBase(e);const country=String(targetCountry||'').trim(),localKey=country?`latest-${hash(userId)}-${hash(country)}`:`latest-${hash(userId)}`,local=await read(e,localKey);
 if(local?.result?.type==='cross-border'&&(!country||local.result.targetCountry===country))return hydratedHome(e,local.result,local.recommendations);
 const {completedAt,records}=await completedRecords(e,userId,country);
 if(!records.length)return countryGuideHome(e,country);
 // The latest record is authoritative even when its matching run yielded no
 // provider. A history or country view must never fall back to an older plan.
 return hydrateAssessmentRecord(e,records[0],completedAt);
}
...
export async function submitCrossBorder(e,identity,input,requestId){
 ...
 const task=(async()=>{
  ...
  if(job?.status==='completed'){await write(e,`latest-${hash(userId)}`,job);await write(e,`latest-${hash(userId)}-${hash(job.result.targetCountry)}`,job);return {result:job.result,home:await hydratedHome(e,job.result,job.recommendations),saved:true};}
  ...
  try{
   ...
   for(const r of job.recommendations){const id=`SPR${job.result.id.slice(3)}-${r.rank}`,professionalIds=(r.professionals||[]).map(person=>person.recordId).filter(Boolean);await upsert(e,TABLES.recommendation,'推荐记录ID',id,{'推荐记录ID':id,'用户ID':userId,'关联客户':[customer.record_id],'关联跨境评估':[assessment.record_id],'关联服务商':[r.providerRecordId],'关联推荐专业人员':professionalIds.length?professionalIds:r.professionalRecordId?[r.professionalRecordId]:[],'匹配维度':r.dimensions,'推荐理由':r.reason,'推荐顺位':r.rank});}
   ...
   job.status='completed';job.result.completedAt=job.commit.time;await write(e,key,job);await write(e,`latest-${hash(userId)}`,job);await write(e,`latest-${hash(userId)}-${hash(job.result.targetCountry)}`,job);
